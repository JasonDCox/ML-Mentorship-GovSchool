...
def kNN(file_location, k):
    ...
    for X in Data:
        ...
        for Y in Data: 
            dis = 0    
            dis_euc = euclidian(X,Y)
            dis_man = manhattan(X,Y)
            dis_min = minkowski(X,Y)
            if indexX == indexY:
                dis_euc = np.inf
                
            if len(top_dis_euc) < k:
                top_dis_class_euc.append(Target[indexY])
                top_dis_euc.append(dis_euc)
            elif dis_euc < max(top_dis_euc):
                top_dis_class_euc.pop(top_dis_euc.index(max(top_dis_euc)))
                top_dis_class_euc.append(Target[indexY])
                top_dis_euc.remove(max(top_dis_euc))
                top_dis_euc.append(dis_euc)
                
            if len(top_dis_man) < k:
                top_dis_class_man.append(Target[indexY])
                top_dis_man.append(dis_man)
            elif dis_man < max(top_dis_man):
                top_dis_class_man.pop(top_dis_man.index(max(top_dis_man)))
                top_dis_class_man.append(Target[indexY])
                top_dis_man.remove(max(top_dis_man))
                top_dis_man.append(dis_man)
                
            if len(top_dis_min) < k:
                top_dis_class_min.append(Target[indexY])
                top_dis_min.append(dis_min)
            elif dis_min < max(top_dis_min):
                top_dis_class_min.pop(top_dis_min.index(max(top_dis_min)))
                top_dis_class_min.append(Target[indexY])
                top_dis_min.remove(max(top_dis_min))
                top_dis_min.append(dis_min)
        ...
        mostCommon = []
        measures = (top_dis_class_euc, top_dis_class_man, top_dis_class_man)
        
        for x in measures: 
            lst = list(str(int(majority(x))) * x.count(majority(x)))
            for y in lst:
                mostCommon.append(y)

        
        predictions[indexX] = [indexX, majority(mostCommon)]
        indexX += 1
...
predictions, Target = kNN(file_name, k)
